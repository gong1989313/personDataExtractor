...
import com.gxq.learn.extractor.enums.FileTypeMode;
import com.gxq.learn.extractor.enums.OptionFile_TYPE;

public class NewNIOUtil implements NewNIoInf {
	/***
	 * 拷贝或者移动文件
	 */
	@Override
		...
		if (srcPath.toFile().exists()) {
			...
			for (int i = 0; i < 3; i++) {
				...
				if (tarPath.toFile().exists()) {
					...
					/***
					 * 移动操作，这里得非常小心,正常情况下，如果两个文件是一样的话， 那么直接删除原始文件就可以了。但是，如果两个文件的一样，并且地址也
					 * 是一样的话，那么就不能删除原始的了，因为就是同一个文件，不能删除的。
					 */
					else if (OptionFile_TYPE.MOVE.equals(type)) {
						if (equalsFile(srcPath.toFile(), tarPath.toFile())) {
							if (!srcPath.toFile().getAbsoluteFile().equals(tarPath.toFile().getAbsoluteFile()))
								try {
	 ...
	 */
	@Override
	public void copeOrMoveDirectory(String src, final String tar, int tierSize, final OptionFile_TYPE type) {
	...
	public List<String> findFilesByType(String dir, String[] keys, boolean isMatchCase) throws IOException {
		...
		Files.walkFileTree(Paths.get(dir), new FindFileUtil(keys, isMatchCase, list, FileTypeMode.TYPES));
		return list;
	}
	...
	public List<String> findFilesByName(String dir, String[] keys, boolean isMatchCase) throws IOException {
		...
		List<String> list = new ArrayList<String>();
		Files.walkFileTree(Paths.get(dir), new FindFileUtil(keys, isMatchCase, list, FileTypeMode.NAMES));
		return list;
	}

	public static void main(String[] args) throws IOException {

		NewNIoInf inf = new NewNIOUtil();
		inf.copeOrMoveFile("e:/cc/dd/11.txt", "e:/XX/xxx/zzz/", OptionFile_TYPE.COPE);
		inf.copeOrMoveDirectory("e:\\BB\\CC\\DD", "e:\\", 1, OptionFile_TYPE.MOVE);
		System.out.println(inf.findFilesByName("D:\\workspace", new String[] { "txt" }, false).size());
